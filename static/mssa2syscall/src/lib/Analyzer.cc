...
string replace(string old, string pattern, string newstr) {
	size_t start_pos = 0;
    while((start_pos = old.find(pattern, start_pos)) != string::npos) {
        old.replace(start_pos, pattern.length(), newstr);
        start_pos += newstr.length();
    }
    return old;
};

map<pair<string, int>, int> getSourceLine2index() {
	int idx = 0;
	map<pair<string, int>, int> sourceLine2index;
	// 打开文件
	ifstream multiPositionPointsFile(MultiPositionPoints);
	if (!multiPositionPointsFile.is_open()) return sourceLine2index;
	// 逐行读取
	while (!multiPositionPointsFile.eof()) {
		string line;
		vector<string> splitRes;
		...
		// DEBUG("line: " << line << "\n");
		splitString(line, splitRes, ":");
		string read_src = splitRes[0];
		string write_src = splitRes[2];
		int read_line = stoi(splitRes[1]);
		int write_line = stoi(splitRes[3]);
		pair<string, int> read_inst = make_pair(read_src, read_line);
		pair<string, int> write_inst = make_pair(write_src, write_line);
		if(sourceLine2index.count(read_inst)==0){
			sourceLine2index[read_inst] = idx;
			idx++;
		}
		if(sourceLine2index.count(write_inst)==0){
			sourceLine2index[write_inst] = idx;
			idx++;
		}
	}
	return sourceLine2index;
};

map<string, BasicBlock*> NewParseDataset(map<pair<string, int>, int> sourceLine2index){
	int idx = 0;
	map<string, BasicBlock*> Dataset;
	...
	for (auto p: GlobalCtx.Modules) {
		...
		for (auto mi = M->begin(), ei = M->end(); mi != ei; mi++) {
			...
			for (auto &inst: instructions(F)) {
				...
				string src = Loc->getFilename().str();
				// cout << "src: " << src << " line: " << line << "\n";
				src = replace(src, "linux/", "");
				pair<string, int> pair_for_match = make_pair(src, line);
				if(sourceLine2index.count(pair_for_match) != 0) {
					// 当前指令在dependent_ops里有对应，记录！
					idx = sourceLine2index[pair_for_match];
					Dataset[to_string(idx)] = inst.getParent();
				}
			}
		}
	}
...
int main(int argc, char **argv) {
	...
	map<pair<string, int>, int> sourceLine2index;
	sourceLine2index = getSourceLine2index();
	map<string, BasicBlock*> Dataset = NewParseDataset(sourceLine2index);
	...
	map<int, pair<string, int>> index2sourceLine;
	for(auto kv : sourceLine2index) {
		int idx = kv.second;
		pair<string, int> sourceLine = kv.first;
		index2sourceLine[idx] = sourceLine;
	}
	...
	J.array([&]{
		for(auto OF:outputFrames){
			J.object([&] {
				...
				J.attribute("source", index2sourceLine[stoi(OF.caseIdx)].first);
				J.attribute("line", index2sourceLine[stoi(OF.caseIdx)].second);
