...
memory_locations = {}
load_insn_num, store_insn_num = 0, 0
...
class MempairResult:
    ...
    def print_all(self):
        ...
        for mempair in self.deduped_mempair.keys():
            # print(mempair[0]+':'+str(mempair[1])+':'+mempair[2]+':'+str(mempair[3]))
            for mempair, typ in sorted(self.deduped_mempair.items()):
                print(mempair[0], mempair[1], mempair[2], mempair[3])
            # if global_result.get(mempair[0]) is None:
            #     global_result[mempair[0]] = {}
            # if global_result[mempair[0]].get(str(mempair[1])) is None:
            #     global_result[mempair[0]][str(mempair[1])] = {mempair[2]: [mempair[3], ]}
            # if mempair[2] not in global_result[mempair[0]][str(mempair[1])]:
            #     global_result[mempair[0]][str(mempair[1])][mempair[2]] = [mempair[3], ]
            # else:
            #     # 检查，去重
            #     global_result[mempair[0]][str(mempair[1])][mempair[2]].append(mempair[3])
...
def inst_process(insn):
    global load_insn_num
    global store_insn_num
    source_locs = []
...
if __name__ == '__main__':
    mssa_srcs = []
    for filename in os.listdir(mssa_dir):
        if filename.startswith('mssa.'):
            mssa_srcs.append(os.path.join(mssa_dir, filename))
    
    index = 0
    for mssa in mssa_srcs:
        memory_locations = {}
        load_insn_num, store_insn_num = 0, 0
        print("Processing file:", mssa)
        insts = []
        ...
        print("Total instructions:", load_insn_num + store_insn_num)
        print("Total load instructions:", load_insn_num)
        print("Total store instructions:", store_insn_num)

        # 处理生成结果
        result = memory_compare(result)
        
        output_path = "result_related" + str(index) + ".txt"
        with open(output_path, 'w+') as f:
            sys.stdout = f
            result.print_all()
            sys.stdout = sys.__stdout__
        print("["+str(index)+"/"+str(len(mssa_srcs))+"] Results are written to", output_path)
        index += 1
    
    # with open("dependent_ops.json", 'w') as output_file:
    #     import json
    #     json.dump(global_result, output_file)
    # print("Results are written to dependent_ops.json.")

    # config2code.print_possibly_incorrect_configs()
    # print("Possibly incorrect config info is printed.")

'''
Processing file: /home/jiakai/KConfigFuzz/static/bc2mssa/output/mssa.122
Instructions processing completed.  28424
Total instructions: 62904
Total load instructions: 31668
Total store instructions: 31236
'''