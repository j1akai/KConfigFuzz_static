...
void outputBBSourceRange(BasicBlock* BB, raw_fd_ostream& outfile) {
    if (!BB) return;
    Function* F = BB->getParent();
    Module* M = F->getParent();
    std::string srcFile = M->getSourceFileName();
    ...
    // 只输出有调试信息的基本块
    if (minLine != UINT_MAX && maxLine != 0) {
        outfile << srcFile << " " << F->getName() << " " << idx << " " << minLine << " " << maxLine << "\n";
    }
}
