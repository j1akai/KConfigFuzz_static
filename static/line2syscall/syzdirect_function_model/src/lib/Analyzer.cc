...
int main(int argc, char **argv) {
  ...
  //             StringRef netlinkInfoItem_SendmsgHandler_name = netlinkInfoItem->SendmsgHandler->getName();
  //             outs() << "|---|--- " << "real sendmsg" << ": " << netlinkInfoItem_SendmsgHandler_name << "\n";
  //           }
  ...
  raw_fd_ostream bb2srcRangeFile(StringRef("./bb2srcRange"), OutErrorInfo, sys::fs::CD_CreateAlways);
  outs() << "start output\n";
  ...
  // 5、该系统调用的handler函数名
  // 6、（新增）将基本块对应的源码区间范围写到 bb2srcRangeFile 中。
  ...
  // 新增：输出basicblock对应的源码范围
  for (auto modulePair : GlobalCtx.Modules) {
    Module* M = modulePair.first;
    for (auto& F : *M) {
      for (auto& BB : F) {
        outputBBSourceRange(&BB, bb2srcRangeFile);
      }
    }
  }
  ...
  {
    if (netlinkInfoItem == nullptr)continue;
    try {
