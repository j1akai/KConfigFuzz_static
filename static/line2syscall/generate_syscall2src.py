...
def GenerateKernelInterface(bitcode_dir, output_dir):
    """
    生成 kernel_signature_full 和 kernelCode2syscall.json 文件。

    参数:
        bitcode_dir (str): Linux .bc 文件所在目录。
        output_dir (str): 生成文件保存的目标目录。
    """
    # 准备输出目录
    os.makedirs(output_dir, exist_ok=True)

    SyzFeaturePath = "/home/jiakai/KConfigFuzz/static/line2syscall/syz-features"
    FunctionModelBinary = "/home/jiakai/KConfigFuzz/static/line2syscall/syzdirect_function_model/build/lib/interface_generator"

    # 生成 syzkaller_signature.txt
    syzkaller_signature_path = os.path.join(output_dir, "syzkaller_signature.txt")
    syzkaller_signature_cmd = f"{SyzFeaturePath} > {syzkaller_signature_path}"
    ExecuteCMD(syzkaller_signature_cmd)
    assert os.path.exists(syzkaller_signature_path) and os.stat(syzkaller_signature_path).st_size != 0, \
        "Fail to generate syzkaller signature!!!"

    # 生成 kernel_signature_full
    kernel_signature_file = os.path.join(output_dir, "../kernel_signature_full")
    generating_cmd = f"{FunctionModelBinary} --verbose-level=4 {bitcode_dir} 2>&1 | tee {os.path.join(output_dir, 'log')}"
    ExecuteBigCMD(generating_cmd)
    assert os.path.exists(kernel_signature_file), "Fail to generate kernel signature!!!"

    # 匹配生成 kernelCode2syscall
    kernel_code_to_syscall = MatchSig(syzkaller_signature_path, kernel_signature_file)

    # 保存 kernelCode2syscall.json
    kernel_code_to_syscall_file = os.path.join(output_dir, "kernelCode2syscall.json")
    with open(kernel_code_to_syscall_file, mode="w") as f:
        json.dump(kernel_code_to_syscall, f, indent="\t")

    assert os.path.exists(kernel_code_to_syscall_file), "Fail to generate kernelCode2syscall.json!!!"

    print(f"Generated files:\n- {syzkaller_signature_path}\n- {kernel_signature_file}\n- {kernel_code_to_syscall_file}")

if __name__ == "__main__":
    # 示例：指定 .bc 文件路径和输出目录
    bitcode_dir = "/home/jiakai/tmp/linux_bc_test"  # 替换为实际的 .bc 文件路径
    output_dir = "/home/jiakai/KConfigFuzz/static/line2syscall/output"    # 替换为实际的输出目录

    GenerateKernelInterface(bitcode_dir, output_dir)
